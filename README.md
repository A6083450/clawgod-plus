...
| **Lean Settings** | Three-level token optimization for `~/.claude/settings.json`. **on**: removes unused tool definitions + disables Workflows/RemoteControl/Artifact. **max**: additionally removes Plan mode, Agent Teams, bundled skills. **off** (default): all tools restored |

> **Lean Settings** are non-destructive and persist across updates. Toggle anytime: `claude --lean-on` / `claude --lean-max` (aggressive) / `claude --lean-off` (default, restore all). To opt out of a single setting, set it yourself (e.g. `"disableArtifact": false`).
