...
const nWrappedActionShape = `${chain}.action(n(async(u)=>{`;

for (const [label, shape] of [
  ['legacy .action(async()=>{', legacyActionShape],
  ['2.1.231 .action(t(async(s)=>{', tWrappedActionShape],
  ['2.1.238 .action(n(async(u)=>{', nWrappedActionShape],
]) {
  ...
  assert.match(match[2], /^\.action\((?:[\w$]+\()?async\([^)]*\)=>\{$/, `action capture group must capture the action opening for ${label}`);
